...
export default function Projects() {
  ...
  return (
    <div className="h-screen flex flex-col bg-shades-1">
        ...
        <div className="w- flex gap-5 font1">
          {category &&
            category.map((category) => (
              <div key={category?.id} onClick={() => setFilter(category.value)}>
                <div
                  className={`group ${
                ...
                >
                  <p className="text-md font-semibold">{category.value}</p>
                </div>
              </div>
            ))}
        </div>
      ...
      <div className="w-full h-auto  bg-shades-1 flex items-center justify-center pr-7 pl-7">
